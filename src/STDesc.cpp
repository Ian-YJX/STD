                         ...
                         double voxel_size)
{
    ...
    for (auto iter = voxel_map.begin(); iter != voxel_map.end(); ++iter)
    {
        ...
        pl_feat[i].r = iter->second.rgb[0] / iter->second.count;
        pl_feat[i].g = iter->second.rgb[1] / iter->second.count;
        pl_feat[i].b = iter->second.rgb[2] / iter->second.count;
        i++;
    }
}
    ...
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr &corner_points)
{
    ...
    else
    {
        ...
        for (size_t i = 0; i < prepare_corner_points->size(); i++)
        {
            ...
            float gray = 0.299 * input_cloud->points[i].r + 0.587 * input_cloud->points[i].g + 0.114 * input_cloud->points[i].b;
